...
const long CO2WarmUp = 240000;                // CO2 sensor warmup time in ms (datasheet says 3 minutes)
...
void loop() {
  // Read oxygen levels, display them appropriately
  //displayFloat(oxygenRead(0), 1, 3);
  //displayFloat(oxygenRead(1), 2, 3);
  //displayFloat(oxygenRead(2), 3, 3);

  // CO2, shows value or countdown to when the sensor's ready if too early
  
  if (millis() < CO2WarmUp) displayFloat((CO2WarmUp - millis())/1000.0, 0, 1); // Warmup countdown (decimal indicates not CO2)
  else displayInt(CO2Level, 0);               // Sensor should be operational

  //if (millis() < CO2WarmUp) displayDashes(0);
...
void CO2Interrupt () {
  ...
  if (digitalRead(CO2Sensor) == HIGH) {
    lastFalling = currentTime; // Mark falling edge ofg PWM
  }
  else {
